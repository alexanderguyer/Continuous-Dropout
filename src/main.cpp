...
void iterate(double alpha, double beta, double upper_bound, double training_rate, int epochs = 20) {
	...
	for (int epoch = 0; epoch < epochs; epoch++) {
		...
		// Shadow input variables with test data
		// input variables
		int num_sets = 10000;
		int read_size = 1000;
		int batch_size = 50;
		...
		cout << setprecision(5) << percent_correct << "/" << flush;

		// Also check accuracy against first 10,000 training data to
		// measure overfitting:
		// Check accuracy after each epoch.
		// Shadow input variables with test data
		// input variables
		...
		percent_correct = (double) num_correct / 10000.0;
		out_file << percent_correct << "\n";

		cout << setprecision(5) << percent_correct << flush;
	}
	cout << " => " << out_file_name.str() << "\n";
}
